...
void Application::Display(void)
{
	...
	case 3:
		...
		// changing the up vector of the camera
	...
	case 5:
		m_pCamera->ResetCamera();
		m_pCamera->SetPosition(AXIS_Z * -15.0f);
		// forcing the cone out of view by changing the planes
		m_pCamera->SetNearFar(vector2(5.0f, 1000.0f));
		break;
	case 6:
		m_pCamera->ResetCamera();
		// forcing the red torus out this time
		m_pCamera->SetPosition(AXIS_Z * -15.0f);
		m_pCamera->SetNearFar(vector2(0.001f, 10.0f));
		break;
	case 7:
		m_pCamera->ResetCamera();
		// inverting the up vector of the camera
