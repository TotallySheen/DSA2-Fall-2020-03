...
void Application::CameraRotation(float a_fSpeed)
{
	...
	// Rotation around X
	quaternion q1 = glm::angleAxis(-fAngleX * sensitivity, vector3(1.0f, 0.0f, 0.0f));
	// Rotation around Y
	q1 = q1 * glm::angleAxis(fAngleY * sensitivity, vector3(0.0f, 1.0f, 0.0f));

	// Setting the target vector based on these rotations
	m_pCamera->SetPositionTargetAndUpward(m_pCamera->GetPosition(), m_pCamera->GetTarget() * q1, AXIS_Y);
